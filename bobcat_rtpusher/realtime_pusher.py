    ...
    for key, value in expect.items():
        if key in data_dict:
            if value != data_dict[key]:
                logging.error("Expect mismatch for %s: expected %s, got %s", key, value, data_dict[key])
                return False
        else:
...
async def process_yaml(data: dict, server: str, speed: float = 1.0):
    ...
    for tentry in data['testdata']:
        ...
        for channel_name, channel_contents in tentry['content'].items():
            (topic, payload) = get_channel_contents(channel_config, channel_name, channel_contents)
            ...
            try:
                while expected:
                    ...
                    if match_response(expect, message_format, message.data):
                        logging.debug("Got expected result on channel %s", message_channel)
                        del expected[message_channel]
                    else:
                        logging.error("Got unexpected result on channel %s = %s", message_channel, message.data)
                        errors += 1
            except asyncio.TimeoutError:
                logging.error("Didn't receive expected result from channel(s): %s", ", ".join(expected))
                errors += len(expected)
        if speed != 0:
...
async def process_csv(filename: str, server: str, speed: float = 1.0):
    ...
    for line in testdata_file:
        ...
        asyncio.get_event_loop().run_until_complete(send_realtime(server, [(topic, payload)]))
...
def main():
    ...
    while True:
        if args.format == 'yaml':
            ...
            process = process_csv(filename=args.filename, server=args.server, speed=args.speed)
        else:
